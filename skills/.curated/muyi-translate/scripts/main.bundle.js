...
// chunk.ts
var parser = new lib_default({ html: true });
function formatChunkUsage(command) {
  return `Usage: ${command} <file> [--max-words 5000] [--output-dir <dir>]`;
}
function runChunkCli(args, command = "chunk.ts") {
...
function chunkMarkdownFile(file, options = {}) {
  ...
  chunks.forEach((chunk, index) => {
    ...
    writeFileSync(join(dir, `chunk-${num}.md`), chunk.blocks.map((block2) => block2.md).join(`
...
function parseChunkCliArgs(args) {
  let file = "";
  let maxWords = 5000;
  let outputDir = "";
  for (let index = 0;index < args.length; index += 1) {
    ...
    if (arg === "--max-words") {
      ...
      if (!value)
        return { error: "Missing value for --max-words" };
      maxWords = parsePositiveInt(value, 0);
      if (maxWords <= 0)
        return { error: `Invalid --max-words value: ${value}` };
      index += 1;
      continue;
    }
    if (arg === "--output-dir") {
...
function trimBoundaryBlankLines(text2) {
...
function buildChunks(blocks, maxWordsPerChunk) {
  ...
  for (const section of sections) {
    const sectionWords = section.reduce((sum, block2) => sum + block2.words, 0);
    if (sectionWords <= maxWordsPerChunk) {
      normalizedBlocks.push(makeBlock("flow", section.map((block2) => block2.md).join(`

`)));
      continue;
    }
    for (const block2 of section) {
      normalizedBlocks.push(...splitOversizedBlock(block2, maxWordsPerChunk));
    }
  }
  const chunks = [];
  let currentBlocks = [];
  ...
  for (const block2 of normalizedBlocks) {
    if (currentWords + block2.words > maxWordsPerChunk && currentBlocks.length > 0) {
      chunks.push({ blocks: currentBlocks, words: currentWords });
      currentBlocks = [block2];
      currentWords = block2.words;
      continue;
    }
    currentBlocks.push(block2);
    currentWords += block2.words;
  }
...
function splitIntoSections(blocks) {
  ...
  for (const block2 of blocks) {
    if (block2.kind === "heading" && current.length > 0) {
...
function splitOversizedBlock(block2, maxWordsPerChunk) {
  if (block2.words <= maxWordsPerChunk)
    return [block2];
  if (block2.kind === "heading" || block2.kind === "thematicBreak" || block2.kind === "html" || block2.kind === "code") {
    return [block2];
  }
  const lines = block2.md.split(`
`);
  if (lines.length <= 1) {
    return [block2];
  }
  ...
  for (const line of lines) {
    ...
    if (bufferWords + lineWords > maxWordsPerChunk && buffer.length > 0) {
      splitBlocks.push(makeBlock(block2.kind, buffer.join(`
`)));
      buffer = [line];
...
function countWords(text2) {
  const cleaned = text2.replace(/[#*`\[\]()>|_~-]/g, " ");
...
// main.ts
function formatScriptCommand(fallback) {
  const raw = process2.argv[1];
  const displayPath = raw ? (() => {
    const relative = path.relative(process2.cwd(), raw);
    return relative && !relative.startsWith("..") ? relative : raw;
  })() : fallback;
  const quotedPath = displayPath.includes(" ") ? `"${displayPath.replace(/"/g, "\\\"")}"` : displayPath;
...
if (args[0] === "chunk") {
  process2.exit(runChunkCli(args.slice(1), `${formatScriptCommand("scripts/main.ts")} chunk`));
}
process2.exit(runChunkCli(args, formatScriptCommand("scripts/main.ts")));
